...
use crate::input::{CursorPosition, PlayerInput, PrevCursorPosition};
use crate::prelude::*;
...
pub fn scroll_camera(
    ...
    mut cursors: Query<(&mut CameraMovementInput, &PrevCursorPosition, &CursorPosition), With<PlayerInput>>,
) {
    if !input.pressed(bindings::MOVE_CAMERA) {
        return;
    }

    for (mut movement, prev_position, curr_position) in cursors.iter_mut() {
        let delta = prev_position.0 - curr_position.0;
        movement.0 = delta;
    }
}